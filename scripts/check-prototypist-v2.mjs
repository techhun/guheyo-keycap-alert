...
function parseCardText(value) {
  const text = clean(value);
  const match = text.match(/\s+((?:Late\s+)?Q[1-4](?:\s*-\s*Q[1-4])?\s+\d{4}|TBC)$/i);
  if (!match) return { product: text, expectedShipping: '' };
  return {
  ...
  await page.waitForFunction(() => {
    ...
    return [...document.querySelectorAll('a')].filter((a) => {
      try {
        ...
        return /prototypist\.notion\.site$/.test(u.hostname)
          && u.pathname.replace(/\/$/, '') !== currentPath
          && (a.innerText || '').trim();
      } catch {
        return false;
      }
  ...
  return page.evaluate((statuses) => {
    ...
    const rows = anchors.map((row) => {
      ...
      return {
        ...
        statusDistance: nearest?.distance ?? Number.POSITIVE_INFINITY
      };
    });

    return { headerCount: headers.length, anchorCount: anchors.length, rows };
  }, STATUSES);
}
  ...
  for (let attempt = 1; attempt <= 2; attempt += 1) {
    ...
    try {
      ...
      if (extracted.headerCount < 4) {
        throw new Error(`${source.label} rendered only ${extracted.headerCount} status headers.`);
      }

      const deduped = new Map();
      for (const raw of extracted.rows) {
        if (!STATUSES.includes(raw.status) || raw.statusDistance > 160) continue;
      ...
      if (rows.length < 5) {
        throw new Error(`${source.label} returned too few mapped rows: ${rows.length}/${extracted.anchorCount}.`);
      }
...
function buildMap(rows) {
  ...
  for (const row of rows) {
    map.set(normalizeUrl(row.itemUrl) || `${row.sourceId}|${clean(row.product).toLowerCase()}`, row);
  }
  return map;
}
...
function sourceCounts(rows) {
  return Object.fromEntries(SOURCES.map((source) => [source.id, rows.filter((row) => row.sourceId === source.id).length]));
}
